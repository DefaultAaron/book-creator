...
**§12 invariant check (HARD — fail closed).** For every section in Batch `$K` that is named in plan §12 as a *consumer* of a cross-section artifact, every producer of that artifact MUST already be at `workflow_status: reviewing` or `complete` AND the producer's acceptance checkpoint must already have been recorded (case i) or §12 amended in place (case ii). If any producer is still `draft` / `planned` / absent, OR the §12 row carries no acceptance-checkpoint marker for it, ABORT — Batch `$K` is not ready. This is belt-and-suspenders against a Phase-3 review that missed an intra-batch / out-of-order producer-consumer relationship; the primary defense is the Phase-3 codex sanity check, but dispatch-time verification fails closed.
