...
- If the produced artifact matches the row above exactly → main session **annotates the row in place** with `(producer <N>.<m>: accepted YYYY-MM-DD via <agreed-sha>)`. `<agreed-sha>` is the producer section's `agreed(...)` commit SHA (never the in-flight lockstep SHA). Per-producer when the row has multiple producers. Each producer carries **exactly one current clause** at any time — on re-AGREED of a previously-annotated producer, REPLACE its existing clause (never append a second).
- If it drifted *within* the row's allowed shape (a name choice, an ordering convention, an optional field added or omitted) → main session **amends the row in place** and adds an inline annotation `(producer <N>.<m>: normalized YYYY-MM-DD via <agreed-sha>)`. Per-producer for multi-producer rows; replace on re-AGREED. Consumer briefs drafted afterward consume the amended row.
