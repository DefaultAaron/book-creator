...
rm -rf .git && git init && git add . && git commit -m "scaffold: book-creator pipeline"
...
**Phase 1 — Research (per chapter).** Main + Gemini + Codex RESEARCH parallel research. Main integrates into `_workflow/research/<chapter_slug>_synthesis.md`.
...
**Phase 4 — Per-section drafting (parallel where independent).** For each batch of independent sections in the DAG: main builds briefs → enforces full-repo clean precondition → writes batch sentinel → dispatches all writers in **one message** (parallel `Agent` tool calls) → copies codex-writer outputs back from worktree → runs structured post-batch validation → removes sentinel.
...
3. **codex-writer hard isolation** — Codex always runs with `--cwd ../<repo-name>-codex-worktree` (a sacrificial git worktree on a long-lived branch). Only the assigned section path is copied back to main. The worktree's `reset --hard` is destructive *only inside the worktree*, never main.
...
| Code-example / asset / figure conventions | Add a short section to book outline §4; reference per chapter |
| Codex / Gemini model overrides | Edit `.claude/agents/codex-{collaborator,writer}.md` and `.claude/agents/gemini-researcher.md` |
