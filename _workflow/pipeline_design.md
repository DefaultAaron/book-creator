...
- **(i) Accepted as-is** — producer matches §12 spec exactly. §12 is **annotated in place** on the artifact row with `(producer <N>.<m>: accepted YYYY-MM-DD via <agreed-sha>)`, parallel to case (ii)'s normalization annotation. `<agreed-sha>` is the producer's `agreed(<chapter>/<section>):` commit SHA — anchored to a commit that already exists at the time the annotation is written, never self-referential to the in-flight lockstep commit. The annotation is what the `draft-batch` step-0 §12 invariant check reads to confirm acceptance happened. **For multi-producer artifact rows, append one `(producer <N>.<m>: ...)` clause per producer** as each one closes; the dispatch-time check requires every producer named in the row to have its own clause. Also logged in STATE.md `do_not_redo` as a session-recovery aid (not load-bearing for the invariant). Commit prefix: `lockstep(<chapter>): §12 acceptance — <artifact-id> producer <N>.<m> — <one-line>`.
- **(ii) Accepted with normalization** — producer drifted *within* §12's allowed shape (clarification only: a name choice, an ordering convention, an optional field added or omitted). §12 is **amended in place** with `(producer <N>.<m>: normalized YYYY-MM-DD via <agreed-sha>)` annotation, again anchored to the producer's `agreed(...)` commit SHA, and per-producer when the row has multiple producers. Consumer briefs drafted afterward consume the amended §12 — they never derive contract shape from "as-built" producer state. Commit prefix: `lockstep(<chapter>): §12 normalization — <artifact-id> producer <N>.<m> — <one-line>`.

**Re-AGREED replacement rule (load-bearing).** Each producer carries **exactly one current annotation clause** on the §12 row at any time. If a producer is reopened (case iii) and later re-AGREED, the new acceptance checkpoint **replaces** the producer's existing clause in place — it does NOT append a second live clause. The replacement clause's `<agreed-sha>` is the *new* `agreed(...)` SHA. Two live clauses for the same producer would let a stale annotation satisfy the dispatch-time invariant after a reopen. Prior clauses remain in git history (visible via `git log -p _workflow/plans/<N>_<chapter_slug>_chapter_plan.md`); only the latest is on the row.

**Latest-`agreed-sha` match (dispatch-time enforcement).** `draft-batch`'s §12 invariant check verifies that each producer's clause `<agreed-sha>` matches the producer's latest `agreed(<chapter>/<producer-section>):` commit SHA in `git log`. A mismatch means a stale clause was not replaced on re-AGREED — ABORT.
