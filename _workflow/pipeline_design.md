...
- **(i) Accepted as-is** — producer matches §12 spec exactly. §12 is **annotated in place** with `(accepted YYYY-MM-DD via <commit-sha>)` on the producer's row, parallel to case (ii)'s normalization annotation. The annotation is what the `draft-batch` step-0 §12 invariant check reads to confirm acceptance happened — without it, dispatch-time has no §12-visible signal. Also logged in STATE.md `do_not_redo`. Commit prefix: `lockstep(<chapter>): §12 acceptance — <artifact-id> — <one-line>`.
