...
5. **Per-section deal-loop** — **asymmetric by writer model**:
   ...
   - **Revisions go through writer dispatch — drafts AND revisions.** Main session direct-edits section content only via `main-direct: writer-overhead` (narrow: spelling, duplicated word, broken Markdown, format artifact — no semantic changes) or `main-direct: writer-unavailable-contingency` (writer dispatch was attempted and blocked by quota / rate-limit / unavailability; exactly one local sentence; no new claim, evidence, stance, or structure; commit body records writer / cause / attempt-ts / retry-window / before / after; STATE.md carries `[contingency-pending-readjudication]` marker until the next writer dispatch or Phase 6 codex adjudicates).
   ...
   - **Producer artifact acceptance checkpoint** — when chapter plan §12 names this section as producer of a cross-section artifact, after AGREED main session records exactly one outcome per artifact: (i) accepted as-is (annotate §12 row with `(producer <N>.<m>: accepted YYYY-MM-DD via <agreed-sha>)`), (ii) accepted with normalization (amend §12 row + add `(producer <N>.<m>: normalized YYYY-MM-DD via <agreed-sha>)` annotation; consumer briefs read the amended §12), or (iii) rejected (reopen producer's deal-loop). `<agreed-sha>` is the producer's `agreed(...)` SHA, never self-referential. Per-producer for multi-producer rows. §12 is the single source of truth — never derive contract shape from "as-built" producer files.
