...
5. **Per-section deal-loop** — **asymmetric by writer model**:
   ...
   - **Producer artifact acceptance checkpoint** — when chapter plan §12 names this section as producer of a cross-section artifact, after AGREED main session records exactly one outcome per artifact: (i) accepted as-is, (ii) accepted with normalization (amend §12 in place with `(normalized YYYY-MM-DD via <commit-sha>)` annotation; consumer briefs read the amended §12), or (iii) rejected (reopen producer's deal-loop). §12 is the single source of truth — never derive contract shape from "as-built" producer files.
